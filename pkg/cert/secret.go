...
func (c *certManager) ensureSecretWithoutRetry(ctx context.Context) (*corev1.Secret, error) {
	...
	now := time.Now()
	minValidUntil := now.Add(wait.Jitter(time.Hour*24*7, 0.5))
	if err := c.certSecretIsValid(secret, now, minValidUntil); err != nil {
...
func (c *certManager) certSecretIsValid(secret *corev1.Secret, now, minValidUntil time.Time) error {
	...
	if _, err := tls.X509KeyPair(serverPem, serverKey); err != nil {
		return errors.Errorf("while matching server cert and key: %w", err)
	}
	if err := serverCert.CheckSignatureFrom(caCert); err != nil {
		return errors.Errorf("while verifying server cert is signed by CA: %w", err)
	}

	if err := certIsValid(caCert, now, minValidUntil); err != nil {
		return err
	}
	if err := certIsValid(serverCert, now, minValidUntil); err != nil {
		return err
	}
...
func certIsValid(c *x509.Certificate, now, minValidUntil time.Time) error {
	if now.IsZero() {
		now = time.Now()
	}
	if minValidUntil.IsZero() {
		minValidUntil = time.Now()
	}
	...
	} else if !c.NotAfter.After(minValidUntil) {
		return x509.CertificateInvalidError{
			Cert:   c,
			Reason: x509.Expired,
			Detail: fmt.Sprintf("certificate expires at %s before minimum valid time %s", c.NotAfter.Format(time.RFC3339), minValidUntil.Format(time.RFC3339)),
		}
	}
	return nil
}
